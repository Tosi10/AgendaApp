...
export const configurePasswordResetEmail = () => {
  ...
  if (__DEV__) console.log('🔧 Firebase Auth configurado para recuperação de senha');
  if (__DEV__) console.log('📧 Emails de recuperação serão enviados automaticamente');
  if (__DEV__) console.log('⏰ Links expiram em 1 hora');
};
...
export const checkPasswordResetEmailSent = async (email) => {
  try {
    ...
    if (__DEV__) console.log(`📧 Email de recuperação solicitado para: ${email}`);
    return true;
  } catch (error) {
